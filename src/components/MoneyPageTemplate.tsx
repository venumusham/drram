      ...
      <div className="relative min-h-screen bg-white">
        ...
        <section className="pt-8 pb-10 px-4 bg-gradient-to-br from-primary-900 to-primary-800 text-white">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">{data.h1}</h1>
            <p className="text-primary-100 text-base md:text-lg mb-5 max-w-2xl">{data.heroSubheadline}</p>
            <div className="flex flex-wrap gap-2 mb-6">
              {data.heroBadges.map((b) => (
                <span key={b} className="bg-primary-700 px-3 py-1 rounded-full text-xs font-semibold">
                  {b}
                </span>
              ))}
            </div>
                ...
                {data.causes && (
                  <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <ul className="text-gray-700 space-y-2 list-disc pl-5">
                      {data.causes.map((c) => <li key={c}>{c}</li>)}
                    </ul>
                  </div>
                )}
                {data.symptoms && (
                  <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <ul className="text-gray-700 space-y-2 list-disc pl-5">
                      {data.symptoms.map((s) => <li key={s}>{s}</li>)}
                    </ul>
                  </div>
                )}
              </div>
        ...
        <section className="py-12 px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-gray-700 mb-6">
              Each technique is selected based on your anatomy, goals, and clinical examination by Dr. Ram Prabhu.
            </p>
            <div className="grid sm:grid-cols-2 gap-4">
              {data.procedures.map((p) => (
                ...
                >
                  <h3 className="text-lg font-bold text-primary-800">{p.title}</h3>
                  <p className="text-sm text-gray-600 mb-3">{p.description}</p>
                  {p.bestFor && <p className="text-sm text-gray-700"><strong>Best for:</strong> {p.bestFor}</p>}
                  {p.duration && <p className="text-sm text-gray-700"><strong>Duration:</strong> {p.duration}</p>}
                  {p.anesthesia && <p className="text-sm text-gray-700"><strong>Anaesthesia:</strong> {p.anesthesia}</p>}
                  {p.stay && <p className="text-sm text-gray-700"><strong>Stay:</strong> {p.stay}</p>}
                  {p.price && <div className="mt-3 text-primary-700 font-bold">{p.price}</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 px-4 bg-primary-50">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-gray-700 mb-6">
              Transparent fixed-package pricing. <strong>No hidden charges.</strong> 0% interest EMI available for 6, 9, and 12 months.
            </p>
            <div className="overflow-x-auto">
              <table className="min-w-full border border-gray-200 text-sm bg-white">
                <thead className="bg-primary-100 text-primary-900">
                  <tr>
                    <th className="px-3 py-2 text-left font-semibold">Procedure Type</th>
            ...
            <p className="text-xs text-gray-500 mt-3">
              Pricing valid for the Kondapur clinic. Includes surgeon fee, anaesthesia, OT charges, and follow-up visits. Excludes pre-op investigations (~₹3,000-5,000).
            </p>
          </div>
        </section>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <ol className="space-y-4">
              {data.walkthrough.map((s) => (
                <li key={s.step} className="bg-white border-l-4 border-primary-600 shadow-sm p-5 rounded">
                  <span className="font-bold text-primary-800">{s.step} — {s.title}</span>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div className="bg-primary-800 p-5 rounded-lg">
                ...
                <p className="text-primary-100 text-sm">National Board of Examinations, New Delhi (2019). Recognised by NMC as equivalent to MCh Plastic Surgery.</p>
              </div>
              <div className="bg-primary-800 p-5 rounded-lg">
                <h3 className="font-bold mb-2">16+ Years Experience</h3>
                <p className="text-primary-100 text-sm">MBBS 2009, DNB 2019. 6,000+ total procedures across the spectrum of cosmetic and reconstructive surgery.</p>
              </div>
              <div className="bg-primary-800 p-5 rounded-lg">
                <h3 className="font-bold mb-2">Member of IAAPS & APSI</h3>
                <p className="text-primary-100 text-sm">Indian Association of Aesthetic Plastic Surgeons and Association of Plastic Surgeons of India. TSMC License No. 66931.</p>
              </div>
              <div className="bg-primary-800 p-5 rounded-lg">
                <h3 className="font-bold mb-2">Modern Techniques</h3>
                <p className="text-primary-100 text-sm">Up-to-date with the latest techniques, equipment, and post-operative protocols. Continuous learning at conferences and workshops.</p>
              </div>
              <div className="bg-primary-800 p-5 rounded-lg">
              ...
              <div className="bg-primary-800 p-5 rounded-lg">
                ...
                <p className="text-primary-100 text-sm">Read patient experiences and outcomes on our verified Google Business Profile.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="faq" className="py-12 px-4">
          <div className="max-w-3xl mx-auto">
            ...
            <FAQAccordion items={data.faqs.map((f) => ({ question: f.q, answer: f.a }))} />
          </div>
        </section>
        ...
        <section id="booking" className="py-12 px-4 bg-primary-50">
          <div className="max-w-lg mx-auto bg-white rounded-lg shadow-lg p-6">
            ...
            <form className="flex flex-col gap-4" action="https://formspree.io/f/mvgabrkw" method="POST">
              ...
              {data.formGradeOptions && data.formGradeOptions.length > 0 && (
                <select
                  ...
                  defaultValue=""
                >
                  <option value="" disabled>Procedure variant (optional)</option>
